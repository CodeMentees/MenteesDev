                    ...
                    >
                      {posts.filter(p => p._id === selectedId).map(post => (
                        <motion.div
                          key={post._id}
                          layoutId={`blog-page-card-${post._id}`}
                          className="flex flex-col rounded-3xl overflow-hidden shadow-2xl relative w-full bg-white border border-gray-200"
                        >
                          <motion.div layoutId={`blog-page-image-${post._id}`} className="w-full h-72 md:h-96 relative shrink-0">
                            <button
                          ...
                          <div className="p-8 md:p-10 flex flex-col flex-grow relative z-10 -mt-20">
                            <p className="text-sm font-bold tracking-widest uppercase mb-3 text-pink-500 bg-white/80 w-max px-3 py-1 rounded-full backdrop-blur-md">Preview</p>
                            <motion.h3 layoutId={`blog-page-title-${post._id}`} className="text-3xl md:text-4xl font-black text-gray-900 mb-6 leading-tight">
                              {post.title}
                            </motion.h3>
                            <motion.div layoutId={`blog-page-content-${post._id}`} className="text-base md:text-lg mb-8 leading-relaxed text-gray-600">
                              <div dangerouslySetInnerHTML={{ __html: post.content?.substring(0, 400) + "..." }} />
                            </motion.div>
                            
                            <div className="mt-auto pt-6 flex flex-col sm:flex-row gap-4 border-t border-gray-100">
                              <button
                                onClick={() => setSelectedId(null)}
                                className="px-8 py-3 rounded-full text-base font-bold transition border-2 border-gray-200 hover:bg-gray-50 flex-1 text-center text-gray-600"
                              >
                                Go Back
                              </button>
                              <button
                                onClick={() => navigate(`/blogs/${post._id}`)}
                                className="px-8 py-3 rounded-full text-base font-bold transition hover:opacity-90 flex-1 text-center bg-pink-600 text-white shadow-lg shadow-pink-200"
                              >
                                Read Full Article
                              </button>
                            </div>
